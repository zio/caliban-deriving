...
class DerivationMacros(val c: blackbox.Context) extends CalibanUtils {
  ...
  private case class DeriveMember(sym: TermSymbol, envType: Type) {
    ...
    def deriveStep(resolveValue: TermName): Tree = Schema.summon(envType, scalaType) { schema =>
    ...
    def deriveStepWithName(resolveValue: TermName): Tree =
      q"($fieldNameStr, ${deriveStep(resolveValue)})"
  }
  ...
  private def deriveStep(
    ...
    val fields =
      outputs.map(o => DeriveMember(o, env).deriveStepWithName(resolveValue))
    ...
    val resolve =
      q"""
         override def resolve($resolveValue: $tpe): ${Step(requestedEnv)} =
           ${deriveStep(info, requestedEnv, resolveValue, outputs)}
      """
